import {
  FaUsers,
  FaBullseye,
  FaBook,
  FaChalkboardTeacher,
  FaGraduationCap,
  FaFlask,
  FaCalculator,
  FaGlobeAmericas,
  FaLaptopCode,
} from "react-icons/fa";

export const courses = [
  {
    title: "৬ষ্ঠ শ্রেণি থেকে ৮ম শ্রেণি",
    icon: <FaUsers className="text-4xl text-cyan-600" />,
    description: "মাধ্যমিক স্তরের জন্য কোর্স",
    subjects: [
      "গণিত",
      "ইংরেজি",
      "বিজ্ঞান",
      "বাংলাদেশ ও বিশ্বপরিচয়",
      "বাংলা",
      "অন্যান্য সকল বিষয়",
    ],
    features: [
      "ছাত্র-শিক্ষক অনুপাত ১:১০",
      "বেসিক ক্লিয়ারেন্স ক্লাস",
      "অতিরিক্ত প্র্যাকটিস সেশন",
    ],
  },
  {
    title: "৯ম - ১০ম শ্রেণি (এসএসসি প্রস্তুতি)",
    icon: <FaGraduationCap className="text-4xl text-cyan-600" />,
    description: "ক্লাস ৯-১০ এর জন্য সম্পূর্ণ কোর্স",
    subjectGroups: {
      common: {
        title: "সকল বিভাগ",
        subjects: ["ইংরেজি", "আইসিটি", "সাধারন গণিত"],
      },
      science: {
        title: "বিজ্ঞান বিভাগ",
        subjects: ["উচ্চতর গণিত", "পদার্থবিজ্ঞান", "জীববিজ্ঞান", "রসায়ন"],
      },
      commerce: {
        title: "ব্যবসায় শিক্ষা বিভাগ",
        subjects: [
          "হিসাববিজ্ঞান",
          "ফিন্যান্স ও ব্যাংকিং",
          "ব্যবসায় উদ্যোগ",
          "সাধারন বিজ্ঞান",
        ],
      },
      humanities: {
        title: "মানবিক বিভাগ",
        subjects: [
          "অর্থনীতি",
          "ভূগোল ও পরিবেশ",
          "পৌরনীতি ও নাগরিকতা",
          "সাধারন বিজ্ঞান",
          "বাংলাদেশের ইতিহাস ও বিশ্বসভ্যতা",
        ],
      },
    },
    features: [
      "সাপ্তাহিক মডেল টেস্ট",
      "বিষয়ভিত্তিক বিশেষ ক্লাস",
      "গাইড লাইন বই সহ",
    ],
  },
  {
    title: "একাদশ - দ্বাদশ (এইচএসসি প্রস্তুতি)",
    icon: <FaBook className="text-4xl text-cyan-600" />,
    description: "ক্লাস ১১-১২ এর জন্য সম্পূর্ণ কোর্স",
    subjectGroups: {
      common: {
        title: "সকল বিভাগ",
        subjects: ["ইংরেজি", "আইসিটি"],
      },
      science: {
        title: "বিজ্ঞান বিভাগ",
        subjects: ["উচ্চতর গণিত", "পদার্থবিজ্ঞান", "জীববিজ্ঞান", "রসায়ন"],
      },
      commerce: {
        title: "ব্যবসায় শিক্ষা বিভাগ",
        subjects: [
          "হিসাববিজ্ঞান",
          "ব্যবসায় সংগঠন ও ব্যবস্থাপনা",
          "ফিন্যান্স, ব্যাংকিং ও বিমা",
          "উৎপাদন ব্যবস্থাপনা ও বিপণন",
        ],
      },
      humanities: {
        title: "মানবিক বিভাগ",
        subjects: ["অর্থনীতি", "ভূগোল", "পৌরনীতি ও সুশাসন", "ইতিহাস"],
      },
    },
    features: [
      "বোর্ড পরীক্ষার সিলেবাস অনুসরণ",
      "প্র্যাকটিক্যাল ল্যাব সুবিধা",
      "নিয়মিত অ্যাসাইনমেন্ট",
    ],
  },
  {
    title: "মেডিকেল ভর্তি প্রস্তুতি (upcoming)",
    icon: <FaFlask className="text-4xl text-cyan-600" />,
    description: "মেডিকেল কলেজ ভর্তি পরীক্ষার জন্য কোর্স",
    subjects: [
      "পদার্থবিজ্ঞান",
      "রসায়ন",
      "জীববিজ্ঞান",
      "ইংরেজি",
      "সাধারণ জ্ঞান",
    ],
    features: [
      "এমসিকিউ সলভিং টেকনিক",
      "নিয়মিত মডেল টেস্ট",
      "স্পেশাল মক টেস্ট সিরিজ",
    ],
  },
  {
    title: "বিশ্ববিদ্যালয় ভর্তি প্রস্তুতি (upcoming)",
    icon: <FaGlobeAmericas className="text-4xl text-cyan-600" />,
    description: "বিভিন্ন বিশ্ববিদ্যালয়ের ভর্তি পরীক্ষার কোর্স",
    subjects: ["গণিত", "ইংরেজি", "সাধারণ জ্ঞান", "বাংলা", "মানসিক দক্ষতা"],
    features: [
      "বিশ্ববিদ্যালয় ভিত্তিক প্রস্তুতি",
      "পূর্ববর্তী বছরের প্রশ্ন ব্যাংক",
      "টাইম ম্যানেজমেন্ট ট্রেনিং",
    ],
  },
  {
    title: "ইঞ্জিনিয়ারিং প্রস্তুতি (upcoming)",
    icon: <FaLaptopCode className="text-4xl text-cyan-600" />,
    description: "বুয়েট ও প্রাইভেট বিশ্ববিদ্যালয় ভর্তি কোর্স",
    subjects: ["পদার্থবিজ্ঞান", "রসায়ন", "গণিত", "ইংরেজি"],
    features: [
      "বুয়েট প্যাটার্ন অনুসরণ",
      "স্পেশাল প্রোবলেম সলভিং ক্লাস",
      "ফিজিক্স ও ম্যাথ ওয়ার্কশপ",
    ],
  },
];
